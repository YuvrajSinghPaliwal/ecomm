...
public class DataLoader implements CommandLineRunner {
    ...
    private void loadBasicEntities() {
        ...
        if (userRepo.count() == 0) {
            ...
            // 10 Customers
            for (int i = 1; i <= 10; i++) {
                users.add(createUser("customer" + i, "cust" + i + "@ecomm.com", USER_ROLE.ROLE_CUSTOMER));
            }
            // 10 Sellers 
            for (int i = 1; i <= 10; i++) {
                users.add(createUser("seller" + i, "seller" + i + "@ecomm.com", USER_ROLE.ROLE_SELLER));
            }
    ...
    private void createOrders(List<Users> customers, List<Product> products, List<Seller> sellers) {
        for (int i = 0; i < 10; i++) {
            ...
            order.setSellerId(sellers.get(i % 10).getUserId());
